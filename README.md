...
There are several directions in this repo:
* [loralib/](loralib) contains the source code for the package 'loralib', which need to be installed to run the examples we provies;
...
* [examples/NLU/](examples/NLU) contains an example implementation of LoRA in RoBERTa and DeBERTa using our package, which produces competitive results on the GLUR benchmark;
...
## Quickstart
    ...
    import loralib as lora
    model = BigModel()
    # This sets requires_grad to False for all parameters without the string "lore_" in their names
...
3. Training bias vector in tandem with LoRA might be a cost-efficient way to squeeze out extra task performance (if you tune the learning rate carefully). While we did not study its effect throughly in our paper, we make it easy to try in 'lora'. You can mark some biases as trainable by passing "all" or "lora_onlu" to 'bias=' when calling 'mark_only_lora_as_trainable'. Remember to pass the corresponding 'bias=' argument to 'lora_state_dict' when saving a checkpoint.
# ==== Before ====
...
# Alternatively, we can train *all* bias vectors in this model, including LayerNorm biases
...
4. Calling 'model.eval()' will trigger the merging of LoRA parametes with the corresponding pretrained ones, which eliminates additional latency for subsequent forward passes. Calling 'model.train()' again will undo the merge. This can be disabled by passing 'merge_weights=False' to LoRA layers.
## Contact
Please contact us or post an issur if your have any questions.
